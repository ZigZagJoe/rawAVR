...
SdFile dirs[DIR_DEPTH];

#define root	dirs[0]
#define cwd		dirs[currDir]
...
int main(void) {
	...
	canUseSD = card.init(SPI_FULL_SPEED, 0) &&
		volume.init(&card) &&
	...
	while(true) {
		...
		if (bisset(PIND, EMPTY)) {
			...
			while (bisset(PIND, EMPTY)) { // ~empty INACTIVE
				bclr(PORTC, IOR);		
				dlen++;				// do something productive while waiting for sync circuit
				_NOP;
...
FUNC_HANDLER(OPEN) {
	...
	if (!openFile.open(cwd, filename, buffer[0]))
...
FUNC_HANDLER(FILE_MD5) {
	...
	if(!md5File.open(cwd, (char*)buffer, O_RDONLY)) 
		SET_ERROR(FAILED_TO_OPEN);
...
FUNC_HANDLER(CHDIR) {
	...
	} else if (((buffer[0] == '/') || (buffer[0] == '\\')) && (buffer[1] == 0)) {
		while (currDir > 0) {
			cwd.close();
			currDir--;
		}
	} else {
		if (currDir == DIR_MAX_DEPTH)
			SET_ERROR(DIR_MAX_DEPTH);
		
		if (dirs[currDir+1].open(cwd,(char*)buffer,O_RDONLY)) {
			currDir++;
		} else 
...
FUNC_HANDLER(DIR) {
	...
	while (((code = cwd.readDir(&p)) > 0) && c < FILES_PER_DIR_PAGE) {
		if (skip && skip--)
			continue;
...
FUNC_HANDLER(BENCH_READ) {
	...
	if (!benchFile.open(cwd,"bench.txt", OPEN_READ))
		SET_ERROR(FAILED_TO_OPEN);
...
FUNC_HANDLER(BENCH_WRITE) {
	...
	if (!benchFile.open(cwd, "bench.txt",OPEN_WRITE))
		SET_ERROR(FAILED_TO_OPEN);
...
FUNC_HANDLER(EXISTS) {
	...
	bool exists = child.open(cwd, (char*) buffer, O_RDONLY);
...
FUNC_HANDLER(DELETE) {
	for (uint8_t i = 0; i < 13 && i < dlen; i++)
		if (buffer[i] == 0 && i > 0) {
			if (buffer[i+1] == 0xDE) {
				if (!SdFile::remove(cwd,(char*)buffer))
					SET_ERROR(OPERATION_FAILED);
				return;
			}
			break;
